...
let bool_of_doc doc path =
  ...
  | Some value -> value
  | None ->
      Printf.eprintf "ash: ash.toml is missing boolean field %s\n"
        (String.concat "." path);
      exit 1
...
let string_of_doc doc path =
  ...
  | Some value -> value
  | None ->
      Printf.eprintf "ash: ash.toml is missing string field %s\n"
        (String.concat "." path);
      exit 1
...
let string_array_of_doc doc path =
  ...
  | Some value -> value
  | None ->
      Printf.eprintf "ash: ash.toml is missing string array field %s\n"
        (String.concat "." path);
      exit 1
    ...
    |> List.map (fun (mount : Agent_box.mount) ->
        ...
        String.concat "\n"
          [
            ...
            "    echo "
            ^ Util.shell_quote
                ("ash: mounted " ^ mount.tag ^ " at " ^ mount.target)
            ^ " >&2";
            "    ;;";
            "  *'\"exitCode\":42'*) ;;";
            "  *)";
            "    echo "
            ^ Util.shell_quote
                ("ash: failed to mount " ^ mount.tag ^ " at " ^ mount.target)
            ^ " >&2";
            "    printf '%s\\n' \"$result\" >&2";
            "    exit 1";
            "    ;;";
