...
Then it reads `$XDG_CONFIG_HOME/$ASH_NAME/config.toml` (falling back to `~/.config/$ASH_NAME/config.toml`, with `ASH_NAME` defaulting to `ash`, or using `--config`). The optional `global.memory` setting selects VM memory in MiB and defaults to 4096. `global.kitty` defaults to false; when true, spawned manifests and attached sessions use `kitten ssh` unless another feature wraps that SSH command. An explicit `--kitty` also enables Kitty for the requested operation. `global.network_bridge` defaults to `ash0`, and `global.qemu_bridge_helper` defaults to `/run/wrappers/bin/qemu-bridge-helper`. `global.nix_store.strategy` selects `shared` (the default) or `image`; `global.nix_store.image_size_mib` defaults to 16384; `global.persist.image_size_mib` defaults to 16384 and sizes the persist image. `global.default_spaces` lists spaces always applied to every VM, with explicit `--space` selections and saved selections extending it (defaults first, deduplicated). `[global.ls]` configures bounded listing operations: `control_timeout_seconds` defaults to 3 seconds for each Virtle control-socket connect/read/write, and `disk_timeout_seconds` defaults to 10 seconds for each per-VM disk scan; both must be positive integers. `--nix-store-strategy`, `--nix-store-image-size-mib`, and `--persist-image-size-mib` override those defaults for one VM; explicit overrides are saved in its `ash-state.toml` and reused by later spawns and regeneration. Pass `--shares-ro-socket` (or the compatibility alias `--ro-store-socket`) to select an existing virtiofsd socket for the consolidated read-only share. An explicit enabled `[portal]` section enables Portal integration. Selected spaces stage their `rw_mounts` and `ro_mounts` beneath the consolidated share roots and turn their `files` into Virtle `write_files` entries. A space may define `extends = ["base", ...]`; ash traverses these dependencies recursively in declaration order, evaluates dependencies before dependents, and evaluates each reachable space once. Unknown spaces and inheritance cycles are fatal configuration errors.

Space selection during evaluation:

- `global.default_spaces` (default: none) is always applied; explicit `-s`/`--space` options and saved selections extend it (deduplicated, defaults first).
- If no `-s`/`--space` option is passed for an existing named VM with `--eval`, `ash` reuses the saved space list on top of the defaults.
...
- If one or more spaces are passed during evaluation, `ash` uses exactly those spaces (plus the defaults) and replaces the saved selection.
...
When the selected flake exposes a matching `homeConfigurations.<name>`, Ash also resolves `homeConfigurations.<name>.activationPackage`, registers its closure together with the NixOS toplevel for the guest database, and appends it to the image after the toplevel import. A `.home-registration` sidecar next to the image records the applied home registration, so home changes only import the missing store paths and never invalidate the toplevel-keyed base image cache; the guest's `home-manager switch` then finds the generation already present and only activates it. Shared-store guests get the same prebuilt closure through the read-only lower-store database.
...
`ash` provisions its SSH identity through QEMU Guest Agent, so the guest should run QGA and respond on the generated `qga.sock`. Passing `--mount-cwd` also requires QGA because `virtle` uses guest commands to bind-mount the workspace. For NixOS guests, enable:
...
Foreground attached launches execute `virtle launch --ssh` directly, allowing `kernel.serial = "console"` to own the terminal. Ash's generated SSH wrapper creates or reuses `<state>/id_ed25519`, installs its public key through QGA, loads Nix registration metadata, restores configured and runtime mounts, and then invokes the selected OpenSSH, Kitty, or Waypipe command with that identity. `--attach --keep` instead starts a background user unit and uses Ash's control-RPC key installation before attaching.
...
For attached flows, Ash creates or reuses `id_ed25519` under the VM state directory and installs it through QGA before attempting authentication. Foreground `virtle launch --ssh` sessions do this in Ash's generated SSH setup wrapper; background-spawned VMs use Ash's equivalent control-RPC installation before attaching.
