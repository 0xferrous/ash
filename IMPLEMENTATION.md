...
For the `image` strategy, Ash resolves the selected toplevel closure plus its `closure-info` output and writes both directly into a sparse ext4 filesystem through the shared `libext2fs` importer. Store paths are written below `/store` in the image, which becomes `/nix/store` when the filesystem is mounted at `/nix`. Writable images use conventional ext4 inode density of roughly one inode per 16 KiB rather than sizing inodes only for the initial closure. The image initially contains store paths but no Nix database. Ash caches a read-only, closure-sized base image under `$XDG_CACHE_HOME/$ASH_NAME/nix-store-images` (falling back to `~/.cache/$ASH_NAME/nix-store-images`, with `ASH_NAME` defaulting to `ash`), keyed by cache format, toplevel, and registration output rather than writable capacity. Creating a VM clones that base into the VM state with `cp --reflink=auto --sparse=always`, using filesystem CoW when available and retaining sparse-copy behavior otherwise, then grows the clone with `resize2fs` to the configured `image_size_mib`. A versioned neighboring `.toplevel` marker records the closure, registration output, and configured size used to initialize each writable image; images with legacy database, directory-layout, or inode-allocation formats require `ash rebuild-db` before reuse. Increasing `image_size_mib` for a stopped VM enlarges the sparse backing file and runs `resize2fs`; shrinking remains an explicit `ash rebuild-db`. When a stopped VM selects a different closure, Ash checks the existing filesystem, opens it through `libext2fs`, skips immutable targets already present, imports only missing paths, and updates the marker after a successful close. Old and guest-added store paths remain available; a failed import is resumable because the old marker is retained and subsequent attempts skip paths already written. The flake exposes `nixosConfigurations.image-reconcile-first` and `nixosConfigurations.image-reconcile-second`; `checks.x86_64-linux.image-store-reconcile` builds both closures, creates an image from the first, reconciles the second into the same image, verifies both toplevels and the updated marker, and finishes with `e2fsck`.
...
Every manifest also adds an Ash mDNS coordinator as a host-side virtle `run` process. It waits for the control socket, obtains the guest's current IPv4 address through the same stable-MAC QGA query used by `ash ls`, and starts the standalone `ash-mdns` executable inside the guest as the transient system unit `ash-mdns.service`. With the shared Nix store strategy, the responder is available beside the host `ash` executable; if that path is absent, the QGA action falls back to `ash-mdns` from the guest `PATH`, allowing image-store guests to include the responder in their own closure. The responder joins `224.0.0.251:5353` on the matching guest interface and publishes `<dns-label>.ash.local`; it probes before claiming the name, sends cache-flush A records and announcements, answers A/ANY questions, detects conflicting A records, and sends a TTL-zero goodbye on shutdown. The coordinator restarts the unit when the address changes and stops it during virtle teardown. DNS-safe lowercase names up to 63 characters are preserved; transformed names receive an eight-hex digest suffix to avoid normalization collisions.
