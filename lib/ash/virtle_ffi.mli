...
(** [parse data] decodes and resolves [data] (TOML or JSON bytes) in-process
    through the virtle library. Returns [Error msg] when the shim library cannot
    be loaded, the manifest is invalid, or resolution fails. *)
...
(** [qemu_argv m ~cid ~incoming] renders the QEMU argv for the manifest with the
    given vsock CID. [incoming] adds "-incoming defer" for state restore. *)

val free : manifest -> unit
(** [free m] releases the manifest handle. Must be called exactly once and only
    after no other call uses [m]. *)

val plan : manifest -> cid:int -> incoming:bool -> (string, string) result
(** [plan m ~cid ~incoming] renders the executable launch plan for the manifest
    as JSON: resolved run processes, the QEMU command, runtime socket paths,
    virtiofs sockets, directories to prepare, stale-socket cleanup files, and
    the allocated vsock CID. [cid] 0 allocates one from the manifest range;
    [incoming] adds "-incoming defer" for state restore. Nothing is spawned by
    this call; execute the returned plan with [Virtle_plan.execute]. *)
