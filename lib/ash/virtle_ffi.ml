...
let parse data =
  let parse =
    load "virtle_manifest_parse"
      (string @-> size_t @-> ptr int64_t @-> ptr (ptr char) @-> returning int)
  in
  ...
  if parse data (Unsigned.Size_t.of_int (String.length data)) handle err <> 0
  then (
...
let resolved_json m =
  let resolved_json =
    load "virtle_manifest_resolved_json"
      (int64_t @-> ptr (ptr char) @-> ptr (ptr char) @-> returning int)
  in
  let string_free = load "virtle_string_free" (ptr char @-> returning void) in
...
let qemu_argv m ~cid ~incoming =
  ...
  in
  let argv_free =
    load "virtle_argv_free" (ptr (ptr char) @-> size_t @-> returning void)
  in
  let string_free = load "virtle_string_free" (ptr char @-> returning void) in
...
let free m =
  let manifest_free =
    load "virtle_manifest_free" (int64_t @-> returning void)
  in
  manifest_free m
...
let with_manifest data f =
  ...
  | Ok m -> Fun.protect ~finally:(fun () -> free m) (fun () -> Ok (f m))
