...
case "$result" in
  ...
  *)
  ...
  in
  (* The wrapper delegates logging to the ash binary's hidden `ash _log`
     command so levels, colors, timestamps and ASH_LOG_LEVEL filtering live in
     one place (lib/ash/log.ml) instead of a second shell implementation. *)
  let ash_bin = Sys.executable_name in
  let content =
    Printf.sprintf
      {sh|#!/bin/sh
set -eu
...
# Generated by ash. Foreground launches prepare the guest here. Background
# launches already completed the same setup before ash reported them started.
if [ "''${ASH_SKIP_GUEST_SETUP:-0}" != 1 ]; then
%s
...
%s
|sh}
      (Util.shell_quote ash_bin) provision_command registration_command
...
let copy ?virtle ~name ~recursive ~verbose ~from_path ~to_path ~source () =
  ...
  (* The generated SSH wrapper provisions this identity through QGA before it
     execs scp. Provisioning here as well issues two immediate guest-exec RPCs,
     which can make QGA reject the wrapper's second request with pid 0. *)
  let identity = ensure_ssh_identity ~name in
...
let attach_running_code ?virtle ~name ~path ~kitty ~waypipe ~plain_wrapper
  ...
  in
  (* Every selected ssh_exec resolves to an Ash-generated wrapper that installs
     this key before executing SSH. Avoid a duplicate back-to-back guest-exec
     here; some QGA sessions reject the wrapper's second request with pid 0. *)
  let identity = ensure_ssh_identity ~name in
...
let render_resolved_manifest inputs =
  ...
  let memory =
    Option.value inputs.memory ~default:(Ash_config.global_memory config)
  in
...
let ash_config ?(runtime = empty_runtime_mount_state) (inputs : manifest_inputs)
  ...
  in
  let fields =
    match inputs.memory with
    | Some memory -> fields @ [ ("memory", Otoml.integer memory) ]
    | None -> fields
  in
  let fields =
...
let load_ash_config ~name =
  ...
  let doc = load_manifest_doc path in
  {
    ...
    memory = Otoml.find_opt doc Otoml.get_integer [ "spawn"; "memory" ];
    ro_store_socket =
...
let resolve_spawn_memory ~name memory =
  let memory =
    match memory with
    | Some _ -> memory
    | None when has_saved_ash_config ~name ->
  ...
  in
  match memory with
  | Some memory when memory <= 0 -> Log.fatal "memory must be greater than zero"
  | _ -> memory
...
let prepare_spawn ?virtle ?name ?user ?ssh ?systemd_ssh_proxy ?ro_store_socket
  ...
  in
  let memory = resolve_spawn_memory ~name memory in
  let virtle = find_virtle virtle in
  let saved =
...
let wait_and_mount (inputs : manifest_inputs) path =
  ...
  ignore
    (install_ssh_key ~virtle:inputs.virtle ~path ~name:inputs.name
       ~user:(manifest_string (load_manifest_doc path) [ "ssh"; "user" ]))
...
let spawn_saved_and_attach ?virtle ~name ~keep ~kitty ~waypipe ~log_level
    ~verbose =
