...
let find_agent_portal_host () =
  ...
  if Sys.file_exists sibling && Util.is_executable sibling then sibling
    ...
    find_exe
      ~hint:
        "install agent-portal-host alongside ash or disable [portal] in the \
         Ash config."
      ~env:"ASH_AGENT_PORTAL_HOST" None "agent-portal-host"
...
let control_socket_guest_action path action =
  let params = Yojson.Safe.from_string (Qga.params action) in
  match control_socket_rpc path ~method_name:"guest-exec" ~params with
  | Some response -> Qga.int_field ~field:"exitCode" response = Some 0
  | None -> false

let run_mdns_publisher ~control_socket ~responder ~name ~mac =
  ...
  let stop_guest_publisher () =
    if Sys.file_exists control_socket then
      ignore
        (control_socket_guest_action control_socket (Qga.mdns_stop_action ()))
  in
  Fun.protect ~finally:stop_guest_publisher (fun () ->
      while not !stopping do
        let discovered =
          try
            match control_socket_vm_stats control_socket ~mac with
            | Some (ip, _, _) -> ip
            | None -> None
          with Yojson.Json_error _ | Invalid_argument _ -> None
        in
        (match discovered with
        | Some address when Some address <> !published ->
            let action = Qga.mdns_start_action ~responder ~name ~address ~mac in
            if control_socket_guest_action control_socket action then (
              Log.info "started guest mDNS publisher %s -> %s" name address;
              published := Some address)
            else Log.warn "failed to start guest mDNS publisher for %s" name
        | _ -> ());
        wait 2.0
      done);
  0
...
let portal_manifest_fields inputs state_dir =
        ...
        portal_environment_files ~user
          ...
          ~nushell:
            (Printf.sprintf
               ...
               endpoint)
      in
      ([ ("write_files", Otoml.TomlTableArray files) ], runs)

let current_executable () =
  try Unix.readlink "/proc/self/exe"
  with Unix.Unix_error _ ->
    let candidate = Sys.executable_name in
    Option.value (Util.find_in_path candidate) ~default:candidate

let mdns_responder_executable executable =
  match Sys.getenv_opt "ASH_MDNS" with
  | Some path -> path
  | None ->
      let sibling = Filename.concat (Filename.dirname executable) "ash-mdns" in
      if Sys.file_exists sibling then sibling
      else Option.value (Util.find_in_path "ash-mdns") ~default:sibling

let mdns_run ~name ~mac ~control_socket =
  let executable = current_executable () in
  Otoml.table
    [
      ( "exec",
        string_array
          [
            ...
            control_socket;
            "--responder";
            mdns_responder_executable executable;
            "--name";
            Mdns.fqdn name;
            "--mac";
            mac;
...
let render_resolved_manifest inputs =
  ...
  in
  let portal_fields, portal_runs = portal_manifest_fields inputs state_dir in
  let runs =
    mdns_run ~name:inputs.name ~mac:network_mac
      ~control_socket:(control_socket_path virtle_state_dir)
    :: portal_runs
  in
  let document =
    match document with
    | Otoml.TomlTable fields ->
        Otoml.TomlTable
          (fields @ portal_fields @ [ ("run", Otoml.TomlTableArray runs) ])
    | _ -> assert false
  in
  let header =
    Printf.sprintf
...
let render_manifest (inputs : manifest_inputs) =
  ...
  let portal_host =
    ...
    | Some portal when portal.enabled && not portal.global ->
        Some (find_agent_portal_host ())
    | Some _ | None -> None
  in
