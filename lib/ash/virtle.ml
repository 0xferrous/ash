...
let portal_profile_file text =
  Otoml.table
    [
      ("guest_path", Otoml.string portal_profile_path);
  ...
  | Some portal ->
      ...
      let profile, runs =
        if portal.global then (
          ...
          let profile =
            Printf.sprintf
              "# Generated by ash.\nexport AGENT_PORTAL_VSOCK=managed:%d\n"
              host_cid
          in
          let run =
            Otoml.table
          ...
          (profile, [ run ])
      in
      [ ("write_files", Otoml.TomlTableArray [ portal_profile_file profile ]) ]
...
let render_resolved_manifest inputs =
  ...
  let document =
    ...
    | Otoml.TomlTable fields ->
        Otoml.TomlTable (fields @ portal_manifest_fields inputs state_dir)
    | _ -> assert false
  in
  let header =
    Printf.sprintf
